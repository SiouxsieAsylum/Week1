...
class Array
  #Array.class_eval do
    def my_uniq
        elements = []
       self.each {|num|  elements << num unless elements.include? num }
       elements
    end
#end
end
...
class Array
  def two_sum
    idxs = []
    self.each_with_index do |elem1,idx1|
        self.each_with_index do |elem2,idx2|
            if idx2 > idx1
               days << [idx1,idx2] if (array[idx1] + array[idx2]).zero?
            end
         end
     end
    idxs
...
class Array
  def median
    ...
    array_s = self.sort
    if array_s.length.odd?
        array_s[array_s.length/2]
    else
        (array_s[array_s.length/2 ] + array_s[array_s.length / 2 - 1]) / 2.0
    end
  end
end
